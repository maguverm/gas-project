...
def agrupar_balance(dff_cont, dff_dem, granularidad):
    ...
    balance = grp_cont[['periodo_str', 'gbtud_cont']].merge(
        grp_dem[['periodo_str', 'gbtud_dem']], on='periodo_str', how='outer').fillna(0)
...
# ════════════════════════════════════════════════════════════════════════════
st.title("⛽ Monitor de Gas Natural — Superservicios")
st.caption(f"Contratos al: {df['fecha_dia'].max().strftime('%d/%m/%Y')} | Demanda al: {df_dem['fecha_registro'].max().strftime('%d/%m/%Y')}")

(tab1, tab2, tab3, tab4,
 tab5, tab6, tab7, tab8, tab9) = st.tabs([
    "📊 Contratación",
    "📅 Contratación Vigente",
    "📈 Análisis Contratación",
    "🏭 Concentración HHI",
    "📊 Demanda",
    "🔍 Análisis Demanda",
    "🔄 Balance de Mercado",
...
# ════════════════════════════════════════════════════════════════════════════
with tab7:
    st.subheader("Balance de Mercado — Contratación vs Demanda")
