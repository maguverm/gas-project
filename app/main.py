...
def agrupar_balance(dff_cont, dff_dem, granularidad):
    # Contratación: agrupar por período (sin sector)
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    total_cont = balance['gbtud_cont'].mean()
    total_dem = balance['gbtud_dem'].mean()
    dif_prom = balance['diferencia'].mean()
    pct_sobrecont = (balance['diferencia'] > 0).mean() * 100

    k1, k2, k3, k4 = st.columns(4)
    k1.metric("Promedio GBTUD Contratado", fmt(total_cont, 1))
    k2.metric("Promedio GBTUD Demanda", fmt(total_dem, 1))
    k3.metric("Diferencia promedio", fmt(dif_prom, 1))
    k4.metric("% Períodos sobrecontratados", fmt(pct_sobrecont, 1) + '%')

    # ── Gráfico superior: Contratado vs Demanda ───────────────────────────────
    fig_bm = go.Figure()
    fig_bm.add_trace(go.Bar(
        name='Contratado', x=balance['periodo_str'], y=balance['gbtud_cont'],
        marker_color='steelblue',
        text=balance['gbtud_cont'].apply(lambda x: fmt(x, 1)),
        textposition='outside', textangle=-90, textfont=dict(size=7)
    ))
    fig_bm.add_trace(go.Bar(
        name='Demanda', x=balance['periodo_str'], y=balance['gbtud_dem'],
        marker_color='goldenrod',
        text=balance['gbtud_dem'].apply(lambda x: fmt(x, 1)),
        textposition='outside', textangle=-90, textfont=dict(size=7)
    ))
    fig_bm.update_layout(
        barmode='group', title='Contratación vs Demanda (GBTUD)',
        xaxis_title='Período', yaxis_title='GBTUD', height=450,
        legend=dict(orientation='v', x=1.02)
    )
    st.plotly_chart(fig_bm, use_container_width=True)

    # ── Gráfico inferior: Diferencia ──────────────────────────────────────────
    balance['label_dif'] = balance.apply(
        lambda r: fmt(r['diferencia'], 1) + ' (' + fmt(r['pct_sobre_demanda'], 1) + '%)', axis=1
    )
    fig_dif = go.Figure()
    fig_dif.add_trace(go.Bar(
        name='Diferencia (Cont - Dem)',
        x=balance['periodo_str'], y=balance['diferencia'],
        marker_color='darkorange',
        text=balance['label_dif'],
        textposition='outside', textangle=-90, textfont=dict(size=7)
    ))
    fig_dif.add_hline(y=0, line_dash='dash', line_color='black', line_width=1)
    fig_dif.update_layout(
        title='Diferencia Contratación − Demanda (GBTUD) y % sobre Demanda',
        xaxis_title='Período', yaxis_title='GBTUD', height=400,
        legend=dict(orientation='v', x=1.02)
    )
    ...
    # Demanda por empresa y tipo_demanda
    ...
    sobrecont = grp_cont_emp[['empresa', 'tipo_demanda', 'gbtud_cont']].merge(
        grp_dem_emp[['empresa', 'tipo_demanda', 'gbtud_dem']],
        on=['empresa', 'tipo_demanda'], how='outer'
    ).fillna(0)
    ...
    for _, emp_row in emp_totals.iterrows():
        ...
        for _, tipo_row in tipos.iterrows():
            filas.append({
                'Empresa / Tipo Demanda': f'   → {tipo_row["tipo_demanda"]}',
    ...
    # ── Agregar a cargar_datos ────────────────────────────────────────────────────
# Agregar esta función junto a cargar_contratos() y cargar_demanda():
...
# ── Agregar a sidebar ─────────────────────────────────────────────────────────
# Cambiar la línea del radio por:
# seccion = st.radio("Sección",
#     ["⛽ Contratación", "📊 Demanda", "🔄 Balance de Mercado", "🔋 Producción"],
#     label_visibility="collapsed")
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    # Orden fijo por producción total histórica
    @st.cache_data
    def orden_operadores_produccion():
        df_p = cargar_produccion()
        orden = df_p.groupby('operador', observed=True)['energia_mbtu'].sum() \
    ...
    with tab_p1:
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            p_operador = st.multiselect("Operador",
                sorted(df_prod['operador'].dropna().astype(str).unique().tolist()),
                placeholder="Todos", key="p_operador")
        with col2:
            p_fuente = st.multiselect("Fuente",
                sorted(df_prod['fuente'].dropna().astype(str).unique().tolist()),
                placeholder="Todas", key="p_fuente")
        with col3:
            p_tipo = st.multiselect("Tipo de producción",
                sorted(df_prod['tipo_produccion'].dropna().astype(str).unique().tolist()),
                placeholder="Todos", key="p_tipo")

        col4, col5, col6 = st.columns(3)
        with col4:
            p_inicio = st.date_input("Fecha inicio",
                value=df_prod['fecha'].min().date(),
                min_value=df_prod['fecha'].min().date(),
                max_value=df_prod['fecha'].max().date(), key="p_fi")
        with col5:
            p_fin = st.date_input("Fecha fin",
                value=df_prod['fecha'].max().date(),
                min_value=df_prod['fecha'].min().date(),
                max_value=df_prod['fecha'].max().date(), key="p_ff")
        with col6:
            p_gran = st.selectbox("Ver por",
                ["Mensual", "Diario", "Trimestral", "Anual"], key="p_gran")
        ...
        dias_prom_p = dfp['fecha'].dt.days_in_month.mean()
        total_gbtud_p = dfp['energia_mbtu'].sum() / (dias_prom_p * 1000)
        k1p.metric("Total GBTUD", fmt(total_gbtud_p, 1))
        ...
        if p_gran == "Diario":
            ...
            dfp_grp['dias_calendario'] = 1
        elif p_gran == "Mensual":
            dfp_grp['periodo'] = dfp_grp['fecha'].dt.to_period('M')
            dfp_grp['dias_calendario'] = dfp_grp['fecha'].dt.days_in_month
        elif p_gran == "Trimestral":
            dfp_grp['periodo'] = dfp_grp['fecha'].dt.to_period('Q')
            dfp_grp['mes'] = dfp_grp['fecha'].dt.to_period('M')
            dfp_grp['dias_calendario'] = dfp_grp['fecha'].dt.days_in_month
        elif p_gran == "Anual":
            dfp_grp['periodo'] = dfp_grp['fecha'].dt.to_period('Y')
            dfp_grp['mes'] = dfp_grp['fecha'].dt.to_period('M')
            dfp_grp['dias_calendario'] = dfp_grp['fecha'].dt.days_in_month

        # Días por período
        if p_gran in ["Trimestral", "Anual"]:
        ...
        # Total por período para variación
        ...
        # Orden fijo — solo los que están en el filtro actual
        ...
        for op in ops_en_grp:
            ...
            fig_p.add_trace(
                go.Bar(name=op, x=data_op['periodo_str'], y=data_op['gbtud']),
                secondary_y=False
            )
        ...
        fig_p.add_trace(
            go.Scatter(
                name='Var% período anterior',
                x=total_per['periodo_str'],
                y=total_per['var_pct'],
                mode='lines+markers',
                line=dict(color='red', width=2),
                hovertemplate='%{x}<br>Var%: %{y:.1f}%<extra></extra>'
            ),
            secondary_y=True
        )
        ...
        for _, row in total_per.iterrows():
            fig_p.add_annotation(
                x=row['periodo_str'], y=row['gbtud_total'],
                text=fmt(row['gbtud_total'], 1),
                showarrow=False, textangle=-90,
                font=dict(size=7, color='black'), yshift=18
            )

        fig_p.update_layout(
            barmode='stack',
            title='Producción (GBTUD) por Operador',
            xaxis_title='Período', height=450,
    ...
    with tab_p2:
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            pa_operador = st.multiselect("Operador",
                sorted(df_prod['operador'].dropna().astype(str).unique().tolist()),
                placeholder="Todos", key="pa_operador")
        with col2:
            pa_fuente = st.multiselect("Fuente",
                sorted(df_prod['fuente'].dropna().astype(str).unique().tolist()),
                placeholder="Todas", key="pa_fuente")
        with col3:
            pa_tipo = st.multiselect("Tipo de producción",
                sorted(df_prod['tipo_produccion'].dropna().astype(str).unique().tolist()),
                placeholder="Todos", key="pa_tipo")

        col4, col5, _ = st.columns(3)
        with col4:
            pa_inicio = st.date_input("Fecha inicio",
                value=df_prod['fecha'].min().date(),
                min_value=df_prod['fecha'].min().date(),
                max_value=df_prod['fecha'].max().date(), key="pa_fi")
        with col5:
            pa_fin = st.date_input("Fecha fin",
                value=df_prod['fecha'].max().date(),
                min_value=df_prod['fecha'].min().date(),
                max_value=df_prod['fecha'].max().date(), key="pa_ff")
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.plotly_chart(pie_chart(dfpa_canacol, 'operador',
                'Producción por Operador (Canacol agrupado)', 'energia_mbtu'),
                use_container_width=True)
        with col2:
            st.plotly_chart(pie_chart(dfpa, 'fuente',
                'Producción por Fuente/Campo', 'energia_mbtu'),
                use_container_width=True)

        # Top 10 fuentes
        st.plotly_chart(bar_chart_top(dfpa, 'fuente',
            'Top 10 Fuentes por Producción', 'energia_mbtu'),
            use_container_width=True)